...
(function (root) {
    ...
    var EN_TO_ZH = {
        ...
        'Edit Provider': '编辑提供方',
        'Provider Name': '提供方名称',
        'Registered Providers': '已注册提供方',
        'Role Assignments': '角色分配',
        'Reveal API keys': '显示 API 密钥',
        'No providers registered yet.': '尚未注册提供方。',
        'Click "Add Provider" to register your first LLM endpoint.': '点击 "添加提供方" 注册你的第一个 LLM 端点。',
        '(auto)': '(自动)',
        'Model': '模型',
        'API Key': 'API 密钥',
        ...
        'Enter absolute path to your project directory, or click Browse to pick from system folders': '输入项目目录的绝对路径,或点击"浏览"从系统文件夹中挑选',
        'Pick a folder': '选择文件夹',
        'Pick this folder': '选用此文件夹',
        'Roots / Drives': '根 / 盘符',
        'Up': '上一级',
        'Go': '前往',
        'Loading roots...': '加载根目录...',
        'Loading...': '加载中...',
        'Failed to load roots:': '加载根目录失败:',
        'Failed to list:': '列出失败:',
        'Double-click to enter a folder, or click Pick to use the highlighted folder.': '双击进入文件夹,或点击"选用"以高亮的文件夹为目标。',
        'You can still pick this path with the button below.': '你仍可以用下方按钮选用此路径。',
        '(empty folder — Pick to use as working dir)': '(空文件夹 — 选用作为工作目录)',
        'Path filled:': '已填入路径:',
        'Browse system folders': '浏览系统文件夹',
        'Each project keeps its own': '每个项目都保留自己的',
        'folder (vector index, memory store, provider registry).': '文件夹(向量索引、记忆库、提供方注册表)。',
        'Switching to a different folder loads': '切换到不同文件夹会加载',
        "that": '该',
        "project's data; switching back restores the previous project's data — nothing is lost.": '项目的数据;切回去会恢复之前项目的数据 — 不会丢失。',
        'Existing project detected.': '检测到已有项目。',
        'Restored': '已恢复',
        'memories and': '条记忆和',
        'indexed files': '个已索引文件',
        'from': '来自',
        'Fresh project.': '全新项目。',
        'No': '没有',
        'folder existed — a new memory store, vector index and provider registry were created. Run "Index Codebase" from Search to populate them.': '文件夹存在 — 新建了记忆库、向量索引和提供方注册表。请到搜索面板运行"索引代码库"以填充它们。',
